...
if(typeof window.webkit != "undefined" && typeof control == 'undefined'){
window.isJudgeIndoorOutdoor = function(p1,p2,p3){
	window.webkit.messageHandlers.isJudgeIndoorOutdoor.postMessage({isJudgeIndoorOutdoor:p1,MIN_RSSI:p2,PERIOD:p3});
}
window.getSystemInfo = function(){
	window.webkit.messageHandlers.getSystemInfo.postMessage('');
}
window.startLocate = function(p1,p2,p3,p4,p5,p6){
	window.webkit.messageHandlers.startLocate.postMessage({type:p1,time:p2,parInfo:p3,isHeadingOpt:p4,isJumpOpt:p5,isMgOpt:p6});
}
window.onUpdateLocation = function(){
	window.webkit.messageHandlers.onUpdateLocation.postMessage('');
}
window.getLocalMaps = function(){
	window.webkit.messageHandlers.getLocalMaps.postMessage('');
}
window.stopLocate = function(){
	window.webkit.messageHandlers.stopLocate.postMessage('');
}
window.jumpMap = function(str){
	window.webkit.messageHandlers.jumpMap.postMessage({mapName:str});
}
}
...
var jsonStr = '{"major":"2740","url":"http://location.powerlbs.com/lbs/ble/B_0000_1610DA5F46E/modelbt.par","version":24,"uuid":"ab8190d5-d11e-4941-acc4-42f30510b408","buildingCode":"B_0000_1610DA5F46E"}';
...
// 定位结果回调
window.sendBleLocateRes = function(msg) {
	...
	if (msgJson.status != 1 || msgJson.type == 'undefined') return;

	// 只处理status = 1和status = 100的情况

	if (msgJson.type == 1) { // type 1室内，0室外
...
// 获取当前手机里面的地图app
window.sendMapRes = function(resStr) {
	globalData.appArr = JSON.parse(resStr).maps;
}

// 跳转到相应地图
window.doJumpApp = function(str) {
	control.jumpMap(str);
}

// 初始化手机控件信息，定位及蓝牙设备
function initControlInfo() {
	// alert('control start info：' + (typeof control));
	// 室内外判断功能初始化
	control.isJudgeIndoorOutdoor(true, -100, 10);
	// 定位功能初始化
	control.startLocate(location_type, 1000, jsonStr, true, true, true);
	// 监听定位结果
	control.onUpdateLocation();
	// 检测本地安装的地图app
	control.getLocalMaps();
}
// 定义全局map变量
var map;
...
function createFMap() {
    ...
    function initMap() {
        ...
        setTimeout(function() {
            // sendBleLocateRes('{\"status\":1,\"floor\":\"B1\",\"x\":\"12948067.735\",\"y\":\"4850754.51\",\"buildingCode\":\"B_0000_1610733D20D\",\"type\":1}');
            initControlInfo();
        }, 2400);
...
function getPoiType() {
  // 获取json数据
  $.getJSON("data/poitype.json", function(data) {
...
//   var x_ = 12948168.620948466 ;
//   var y_ = 4850452.723823964 + i * 0.1;
 //  var gid_ = 'B2';
//   if (i == 500) {
//   gid_ = 'B1';
//   } else if (i == 1000) {
...
//隐藏导航但不停止语音
function hideNaviPageNotResetAudio() {
  isNaving = false;
  clickType = 1;
  $('.navi-bottom-tip').hide();
  $('.navi-header').hide();

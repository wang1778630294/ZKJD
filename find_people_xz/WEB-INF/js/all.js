...
(function(exports) {
	...
	*/
	...
	var sessionId = spath1.split("=")[1];
	
	var spath2 = spath.split("&")[2];
	
	var avatarUrl = spath2.split("=")[1];
	...
	var fristLocations = null;

	/*
		创建websocket向后台传递openId和sessionId;拿到定位信息绘制定位坐标;
	*/
	createWebsocket();
	function createWebsocket(){
		var socket = new WebSocket("wss://xzdqnavi.powerlbs.com/wechat/locateShare");
	...
	　　   //想要验证是否连接成功可以给后台发一个消息
			...
			var aobj = JSON.stringify(obj);

	　		socket.send(aobj);

	　　　// 这个事件监听，可以监听后台返回来给你的消息
			socket.onmessage = function(res) { 
				if (globalData.is_mapload) {
					if (res.data.substring(0,4)=="erro") {
						clearInterval(socket_timer);
						commonAlert("您不能加入该房间,当前房间人数达到最大上限");
						return;
					}
					...
					if (res.data.substring(0,4)!="join"&&res.data.substring(0,4)!="quit"&&res.data.substring(0,4)!="erro") {
					
						pointdata = JSON.parse(res.data);
						
					}
				
					
					if (res.data.substring(0,4)=="join") {
						for (var i=0; i< avataUrlArr.length;i++) {
							map.removeLocationMarker(avataUrlArr[i]);
						}
						$(".container").empty();
					...
					if (res.data.substring(0,4)=="quit") {
	...
	　　　socket.onclose = function(str) { 
				...
				socket_timer = setInterval(createWebsocket,5000);
	...
	function createFindIcon(obj,pointdata){
		quitData = pointdata;
		/* 
		for (var i=0; i< avataUrlArr.length;i++) {
			map.removeLocationMarker(avataUrlArr[i]);
		...
		*/
		...
		if (pointdata) {
			...
			$("#num").html(pointdata.length)	


			for (var i=0; i<pointdata.length;i++) {
					
				
				fristLocations = pointdata;
				
				base64(pointdata,i);
				function base64(pointdata,i){
					
					convertImgToBase64(pointdata[i].user.avatarUrl, function(base64Img){
					  // Base64DataURL
					if (pointdata[i].user.openId == obj.openId) {
						if (pointdata[i].floor) {
							myLocPoint = {
								x: pointdata[i].x,
								y: pointdata[i].y,
								floor: pointdata[i].floor,
								avatarUrl: base64Img
							}
						}
					}
					
					if (myEndOpenId&&pointdata[i].user.openId == myEndOpenId) {
						if (pointdata[i].floor) {
							myEndLocPoint = {
								x: pointdata[i].x,
								y: pointdata[i].y,
								floor: pointdata[i].floor,
								avatarUrl: pointdata[i].user.avatarUrl
							}
						}
					}
					
					if (avataOpenidArr.indexOf(pointdata[i].user.openId)==-1) {
						avataUrlIconArr[i] = null;
						avataUrlIconArr[i] = document.createElement("div");
						avataUrlIconArr[i].setAttribute("class","weheader");
						
						var img = document.createElement("img");
					
						img.src = pointdata[i].user.avatarUrl;
						
						avataUrlIconArr[i].appendChild(img);
						$(".container").append(avataUrlIconArr[i]);

						avataOpenidArr.push(pointdata[i].user.openId);
						
						avataUrlIconArr[i].onclick = function() {
							
							myEndOpenId = pointdata[i].user.openId;
							
							var first_loc = firstLoc(fristLocations,myEndOpenId);
							
							if (pointdata[i].user.openId == openId) {
								
									commonAlert('你不能导航到自己');

									return;
							}
							if (myLocPoint.floor == undefined) {
								commonAlert("获取不到您当前的位置,请检查您的网络环境");

								return;
							}
							
								
							if (first_loc.floor == undefined) {
								commonAlert("获取不到您好友当前的位置信息");
								...
								layer.open({
									...
									yes: function(index, layero) {
										...
										user_timer = setInterval(function(){
											...
											hideRoute();
											hideClickModelInfo();
											drawLine(myLocPoint,myEndLocPoint);
					...
					var pointdatax = pointdata[i].x ? pointdata[i].x : 0;
					var pointdatay = pointdata[i].y ? pointdata[i].y : 0;
					var pointdatafloor = pointdata[i].floor ? pointdata[i].floor : "B2";
							
					var msg = {
						x: pointdatax,
						y: pointdatay,
						groupID: floorArr[pointdata[i].floor],
						avatarUrl: pointdata[i].user.avatarUrl
					}

					var msg11 = {
						x: parseFloat(pointdatax),
						y: parseFloat(pointdatay),
						groupID: floorArr[pointdatafloor],
					}
					if (avataOpenidIconArr.indexOf(pointdata[i].user.openId)==-1) {
						
						avataUrlArr[i]= new fengmap.FMLocationMarker({
							url: msg.avatarUrl,
							//设置图片显示尺寸
							size: 30,
							//设置图片高度，默认是5
						...
						if (pointdata[i].user.openId == obj.openId) {
							find_marker = avataUrlArr[i];
						}

						map.addLocationMarker(avataUrlArr[i]);
						
						avataOpenidIconArr.push(pointdata[i].user.openId);
						
					}
					
					// 更新路径	
					//updateNaviFirend(msg11);
					
					avataUrlArr[i].setPosition({
						//设置定位点的x坐标
						x: msg.x,
						//设置定位点的y坐标
						y: msg.y,
					...
					if (globalData.isFirstLoction == 0 && msg11.floor) {
				  ...
				  //       }
					...
					// 如果楼层切换
					if (myLocPoint.floor && floorArr[myLocPoint.floor] != globalData.last_located_floor_id) {
						//moveToCenter(msg);
						// map.moveTo(msg11);
						moveToCenter(msg11);
				  ...
				  //       }
						...
						// 记录上次的楼层ID
						globalData.last_located_floor_id = msg.groupID;
					}
	...
	function drawLine(myLocPoint,pointdata){
		navi.setStartPoint({
			x: myLocPoint.x,
			y: myLocPoint.y,
			height: 0,
			groupID: floorArr[myLocPoint.floor],
			url: myLocPoint.avatarUrl,
			size: 10
		});
		...
		if (pointdata.floor) {
			...
			navi.setEndPoint({
				...
				y: pointdata.y,
				height: 0,
				groupID: floorArr[pointdata.floor],
				url: pointdata.avatarUrl,
				size: 10
			});

			// 画导航线
			navi.drawNaviLine();
			showStartAndEndIcon(floorArr[myLocPoint.floor], floorArr[pointdata.floor]); // 在楼层控件旁边添加起点、终点图标
		}
	}
	...
	function convertImgToBase64(url, callback, outputFormat){
	  var convertImgToBase64_canvas = document.createElement('CANVAS'),
	  convertImgToBase64_ctx = convertImgToBase64_canvas.getContext('2d'),
	  convertImgToBase64_img = new Image;
	  convertImgToBase64_img.crossOrigin = 'Anonymous';
	  convertImgToBase64_img.onload = function(){
		convertImgToBase64_canvas.height = convertImgToBase64_img.height;
		convertImgToBase64_canvas.width = convertImgToBase64_img.width;
		
		/*
		var arcimgwidth = convertImgToBase64_img.height/2;
		convertImgToBase64_ctx.save();
		convertImgToBase64_ctx.beginPath();
		convertImgToBase64_ctx.arc(arcimgwidth, arcimgwidth, arcimgwidth, 0, Math.PI * 2, false);
		convertImgToBase64_ctx.clip();
		*/
		
		convertImgToBase64_ctx.drawImage(convertImgToBase64_img,0,0);
		var convertImgToBase64_dataURL = convertImgToBase64_canvas.toDataURL(outputFormat || 'image/png');
		callback.call(this, convertImgToBase64_dataURL);
		convertImgToBase64_canvas = null; 
	  };
	  convertImgToBase64_img.src = url;
	}
...
function firstLoc(pointdata,myEndOpenId){
	var first_location = {};
	for (var i=0;i<pointdata.length;i++){
		if (myEndOpenId&&pointdata[i].user.openId == myEndOpenId) {
			if (pointdata[i].floor) {
...
function updateLocMarkerPosFird(coord) {
	find_marker.setPosition({
		// 设置定位点的x坐标
		x: coord.x,
		// 设置定位点的y坐标
		y: coord.y,
...
//
function updateNaviFirend(mapCoord_) {
	if (!isNaving_fir) return;
	...
	if ((dis == null || dis >= globalData.constraintDis) && !globalData.isRecalcRoute) {
		...
		//updateLocMarkerPosFird(mapCoord_, globalData.cur_direction);
